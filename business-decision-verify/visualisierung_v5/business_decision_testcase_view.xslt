...
<!--
XSLT Transformation für die möglichen Testfälle einer Business-Decision im Format business_decision_v1.n.xsd
...
<xsl:template match="BusinessDecisionTest">
	    <html>
			<head>
				<title>Business Decision <xsl:value-of select="decisionId" /> - Testcases</title>
			...
			<body>
				...
				<br />
				<br />
				<h2>Testfälle dieser Business Decision</h2>					
				<br />
					...
					<thead>
						<tr>
							...
							<xsl:element name="th">
								 ...
								 <xsl:attribute name="colspan"><xsl:value-of select="count(decisionParameter)+2" /></xsl:attribute>
								 <xsl:text>Test Cases </xsl:text><xsl:value-of select="@decisionName" />
							</xsl:element>
						</tr>
						<tr>
						...
						<tr>
							...
							<th class="th_conclusion"><xsl:value-of select="decisionConclusion" /></th>
						</tr>			
					</thead>
					<tbody>
						<xsl:for-each select="testCase">
							<tr>
								...
								<th>
									<xsl:value-of select="expectedOutput"/>
								</th>
							</tr>							
						</xsl:for-each>
					</tbody>
